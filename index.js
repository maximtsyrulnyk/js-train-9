...
function findIndexByProperty(arr, name) {
  return Array.isArray(arr) ? arr.findIndex(obj => obj.name === name) : -1;
...
function findLastEvenNumber(arr) {
  ...
  ? arr.reverse().find((num) => num % 2 === 0) || 'Парних чисел не знайдено'
    : 'Вхідний параметр має бути масивом';
...
function getArrayKeys(arr) {
  ...
  if (!Array.isArray(arr)) {
   ...
   // Використовуємо метод keys() для отримання ітератора ключів масиву та конвертуємо його в масив.
 ...
 */
 const getPositiveArrayValues = (arr) => {
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
  if (!Array.isArray(arr)) {
    return 'Вхідний параметр має бути масивом';
  }
  // Використовуємо метод filter() для отримання масиву лише з додатніми значеннями
 ...
 */
 const removeAndDouble = (arr, startIdx) => {
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо помилку
  if (!Array.isArray(arr)) {
    return 'Вхідний параметр має бути масивом';
  }

  // Використовуємо метод splice() для видалення елементів масиву починаючи з заданого індексу
  arr.splice(startIdx);

  // Використовуємо метод map() для подвоєння кожного елемента масиву
 ...
 */
 const sumAndPrint = (arr) => {
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо помилку
  if (!Array.isArray(arr)) {
    return 'Вхідний параметр має бути масивом';
  }

  // Використовуємо метод reduce() для обчислення суми елементів масиву
  const sum = arr.reduce((acc, value) => {
    // Використовуємо метод forEach() для виведення кожного елемента масиву в консоль
    console.log(value);
    return acc + value;
  }, 0);
...
const flattenSquareAndFilter = (arr, threshold) => {
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо помилку
  if (!Array.isArray(arr)) {
    return 'Вхідний параметр має бути масивом';
  }

  // Використовуємо метод flat() для вирівнювання масиву
  // Використовуємо метод map() для підняття кожного елемента масиву до квадрату
  // Використовуємо метод filter() для фільтрації елементів, які більше за порогове значення
  const filteredSquaredArray = arr.flat().map(element => element ** 2).filter(element => element > threshold);
 ...
 */
 const getLastElementsSumAndPrint = (arr, n) => {
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
  if (!Array.isArray(arr)) {
    return 'Вхідний параметр має бути масивом';
  }
  ...
  return arr.slice(-n).reduceRight((sum, element) => {
    console.log(element);
    return sum + element;
  }, 0);
};
 ...
 */
const insertElementAt=(arr, index, element) => {
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
  if (!Array.isArray(arr)) {
    return 'Вхідний параметр має бути масивом';
  }
...
const expandArrayByN = (arr, n) => {
  ...
  const expandedArray = arr.flatMap(number => Array(n).fill(number)); // Використовуємо метод flatMap для створення нового масиву, де кожне число повторюється n разів
  return expandedArray; // Повертаємо розширений масив
}; 
...
function findDuplicateElements(arr) {
  ...
  // Створюємо Set, щоб зберегти унікальні елементи
  const uniqueElementsSet = new Set();
  // Створюємо Set, щоб зберегти дубльовані елементи
  const duplicateElementsSet = new Set();
   // Використовуємо цикл для проходження через елементи масиву
   for (const element of arr) {
    // Якщо елемент уже є в uniqueElementsSet, то він є дубльованим
    if (uniqueElementsSet.has(element)) {
      duplicateElementsSet.add(element);
    } else {
      // Якщо елемент не є унікальним, додаємо його до uniqueElementsSet
      uniqueElementsSet.add(element);
    }
  }
  // Повертаємо новий масив з дубльованими елементами
  return Array.from(duplicateElementsSet);
}
...
function capitalizeWords(arr) {
  ...
  const capitalizedArray = arr.map((word) => {
    // Перетворюємо першу літеру слова у верхній регістр та додаємо до неї всі символи слова крім першого
    return word.replace(/(?:^|\s)\S/g, (char) => char.toUpperCase());
  });
  ...
  */
  function calculateTotalPrice(arr) {
    ...
    const totalPrice = arr.reduce((sum, product) => {
      // Перевіряємо, чи у кожного товару є властивість price та чи вона є числовою
      if (product && typeof product.price === 'number') {
        // Додаємо ціну товару до загальної суми та повертаємо результат
        return sum + product.price;
      } else {
        // Якщо властивість price відсутня або не є числовою, ігноруємо товар
...
function findLastNegativeNumberIndex(arr) {
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо -1
  if (!Array.isArray(arr)) {
    return -1; // Повертаємо -1, якщо вхідний параметр не є масивом
  }
